...
Remove-Item -Path $(Join-Path $(Get-DesktopDir) "$ShortcutDisplayName.rdp") -EA SilentlyContinue; 

# Remove icon from start
Remove-Item -Path $(Join-Path $(Get-StartDir) "$ShortcutDisplayName.rdp") -EA SilentlyContinue
