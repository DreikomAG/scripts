        ...
        Try { 
            ...
            ForEach ($Dir in $Dirs) {
                ...
                Try {
                    ...
                    $ACL.SetOwner([System.Security.Principal.NTAccount]"$Domain\$Owner")
...
function SetPermissions ($Path, $Domain, $Group, $Level, $Clean, $Delete) {
    if ($null -eq $Domain) {
        $Domain = (Get-ADDomain).NetBIOSName
    }
            $Permissions = "$Domain\$Group", "SYSTEM:FullControl"
            $AccessRules = @()
            $Owner = "domänen-admins"
            ForEach ($Perm in $Permissions.Split(",")) {
                $Group = $Perm.Split(":")[0]
